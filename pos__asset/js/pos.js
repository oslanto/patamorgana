...
function loadItems() {
    ...
    if (get('spositems')) {
        ...
        if(Settings.java_applet == 1) {
            ...
            bill_data += "Bill" + "\n\n";
            order_data += "Order" + "\n\n";
            //bill_data += $('#select2-spos_customer-container').text() + "\n\n";
            //bill_data += " \x1B\x45\x0A\r\n ";
            //order_data += $('#select2-spos_customer-container').text() + "\n\n";
            ...
            // var pos_customer = '<h5>'+$('#select2-spos_customer-container').text()+'</h5>';
            var pos_curr_time = '<h5>'+date(Settings.dateformat+' '+Settings.timeformat, time)+'</h5></span>';
            $("#order_span").prepend(style+pos_head+'<h4>Order</h4>'+pos_customer+'<h5>By: '+username+'</h5>'+pos_curr_time);
            $("#bill_span").prepend(style+pos_head+'<h4>Bill</h4>'+pos_customer+'<h5>By: '+username+'</h5>'+pos_curr_time);
        ...
        $.each(spositems, function () {
            ...
            if(Settings.java_applet == 1) {
                ...
                var oprTr = '<tr class="bb row_' + item_id + '" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + ' (' + item_code + ')</td><td>' + item_qty +'</td></tr>';
        ...
        if(Settings.java_applet == 1) {
            ...
            if(order_discount > 0) {
                bill_totals += '<tr class="bb"><td>'+lang.order_discount+'</td><td style="text-align:right;">'+formatMoney(order_discount)+'</td></tr>';
            }
            if (order_tax != 0) {
